...
<template>
    <div>
        ...
        <div class="flex min-h-screen">
            <Sidebar/>
            <div class="flex-1">
                <div class="flex flex-col h-screen bg-cyan-100">
                        ...
                        <div class="shrink-0 flex justify-between">
                            ...
                            <div class="flex items-center px-3">
                                <Menu as="div" class="relative z-10">
                                    <MenuButton
                                        class="hover:bg-gray-400 ring-2 focus:ring-set-2 ring-gray-400 rounded-full">
                                        <UserIcon class="h-6 w-6 inline ml-2 rounded-full" />
                                        <a class="text-sm text-black font-medium rounded-md mr-3"> {{
                                    ...
                                    <transition enter-active-class="transition transform duration-100 ease-out"
                                        ...
                                        <MenuItems
                                            class="origin-top-right mt-2 focus:outline-none absolute right-0 bg-black overflow-hidden rounded-md shadow-lg border border-gray-600 w-48">
                                            <MenuItem v-slot="{ active }">
                                            <Link :class="{ 'bg-gray-500': active }" :href="route('logout')" method="post"
                                                as="button" class="w-full text-left block px-4 py-2 text-sm text-white">
                                            Cerrar Sesión</Link>
                                            </MenuItem>
                                        </MenuItems>
                                    </transition>
